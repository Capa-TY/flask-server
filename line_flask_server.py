...
def get_openrouter_response(user_message):
    ...
    data = {
        "model": "nvidia/nemotron-nano-12b-v2-vl:free",   # 可以更換其他模型
        "messages": [
        {"role": "system", "content": "你是一位親切專業的財經顧問， 擁有豐富的投資經驗和市場洞察力。你的特點是用幽默詼諧、淺顯易懂的語言，將複雜的股市分析轉化為一般投資人能快速理解的觀點，並且回覆長度盡量控制在100字以內且務必用繁體中文回答，能搭配笑話、表情符號、表格統整更好，重點是提供有價值的洞見並以搞笑輕鬆的回應讓使用者從問題中成長"},
